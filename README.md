...
- **Style Dictionary:** Genre, subgenre, tone, POV, tense, pacing, protagonist type/traits, and novel tags are backed by a seeded dictionary where every term carries a definition and an example, following a NovelUpdates-style taxonomy. Multi-value axes (genre, subgenre, tone, protagonist, tags) use chip pickers — click terms to combine several ("Fantasy / Xianxia", "Trickster, Female Protagonist", "Gender Bender, Reincarnation") or type your own. Protagonist archetypes and traits (gender, wealth) merge into one picker; story tropes like Gender Bender, Reincarnation, Transmigration, System, and Villainess live in the Novel Tags axis. A management page lets authors add their own terms, and every resolved term's definition/example rides into generation prompts.
...
At this review:
- **`pytest`:** **110 passed, 1 skipped** in ~22 s (PostgreSQL lease concurrency test skipped cleanly under SQLite fallback). Suites cover lease-heartbeat behavior, stale-job reaper policy, canon extraction/review/commit flow (including character-state updates), relevance-ranked retrieval, prompt-quality sizing, generalized continuity detection, provider retry semantics, and the style dictionary (including protagonist traits and novel tags).
